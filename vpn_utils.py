...
class Connection:
    ...
    def __init__(self, login: str = LOGIN, password: str = PASSWORD, host: str = HOST):
        self.login = login
    ...
    def login_api(self) -> bool:
        ...
        try:
            response = self.ses.post(f"{self.host}/login", json=data, timeout=10)
    ...
    def add_client(self, days: int) -> Optional[str]:
        ...
        try:
            response = self.ses.post(f"{self.host}/addClient", json=data, timeout=10)
    ...
    def print_inbounds(self) -> None:
        ...
        if not self.ensure_login():
            return
        data = self.list_inbounds()
        logging.info("Inbounds:\n%s", json.dumps(data, indent=2, ensure_ascii=False))

    def create_inbound(self, user_id: int, is_trial: bool = False) -> Optional[Dict[str, Any]]:
        ...
        used_ports = {
            int(inb.get("port")) for inb in existing if str(inb.get("port", "")).isdigit()
        }
        ...
        stream_settings = {
            ...
            "realitySettings": {
                ...
                "maxTimediff": 0,
                "shortIds": [
                    "47595474", "7a5e30", "810c1efd750030e8", "99",
                    "9c19c134b8", "35fd", "2409c639a707b4", "c98fc6b39f45"
                ],
                "settings": {
        ...
        try:
            ...
            if response.status_code == 200:
                logging.info("Inbound создан для user_id=%s на порту %s ✅", user_id, port)
                return {"uuid": client_uuid, "port": port}
            logging.error("Ошибка создания inbound: %s — %s", response.status_code, response.text)
        except requests.RequestException as e:
    ...
    def update_client(self, client_uuid: str, days: int = 30) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                logging.info("Клиент %s продлён до %s ✅", client_uuid, expiry_time)
                return True
